...
void readATOM( char *str, struct atom_rec *atrec )
{
	...
	int startp = 21;
	if( !( str[startp] >= '0' && str[startp] <= '9') )
		startp=22;

	j = 0;
	for( i = startp; i < 27; i++, j++ )
		buffer[j] = str[i];
	buffer[27-startp] = '\0';
...
void printATOM( FILE *toFile, int bead, int res, struct atom_rec *atrec, double aux, int write_hex)
{
	...
	else if( strlen(tbuf) > 5 )
	{
		sprintf(tbuf, "%d", bead  );
		char *ptr_to = tbuf+strlen(tbuf)-5;
		while( *ptr_to == '0' )
			ptr_to += 1;
		if( ! *ptr_to )
			fprintf(toFile,"    0");
		else
		{
			if( strlen(ptr_to) < 5 )
			{
				for( int x = 0; x < 5 - strlen(ptr_to); x++ )
					fprintf(toFile," ");
			}
			fprintf(toFile,"%s", ptr_to);
		}
	}
	fprintf(toFile," " );
