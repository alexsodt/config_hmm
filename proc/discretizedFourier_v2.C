...
#define N_BINS_MOLDIST 100
		...
		double wrapto = 0;
                 ...
                 double moldist[N_BINS_MOLDIST*3];
                 memset( moldist, 0, sizeof(double) * N_BINS_MOLDIST*3 );
...
//                              while( tz >= 1 ) tz -= 1;
                              ...
                              if( zb >= 2*N_BINS_MOLDIST ) zb = 2*N_BINS_MOLDIST-1;
                              moldist[N_BINS_MOLDIST+zb] += 1;
		}
                 ...
                 for( int zb = 0; zb < nbins; zb++ )
                 {
                         double zv = (zb+0.5) / (double)N_BINS_MOLDIST;
                          ...
                          for( int iz = zlow; iz < zhigh; iz++ )
                          {
                                  ...
                                  int iiz = iz;
                                  while( iiz < -N_BINS_MOLDIST ) iiz += 3*nbins;
                                  while( iiz >= 2*nbins ) iiz -= 3*nbins;

                                  lchi2 += moldist[N_BINS_MOLDIST+iiz] * (dz) * (dz);

//				printf("%lf %lf\n", (iz+0.5) / nbins, moldist[N_BINS_MOLDIST+iiz]  );
                          }
...
*/
		...
		// first a pass using the tail atoms gets the midplane approximately and assigns the leaflet.
		// the next pass collects with the leaflet assigned.
		//
		for( int pass = 0; pass < 2; pass++ )
		{
			...
			// first do FT from tails, then do assignment.
			//
			for( int sub_pass = 0; sub_pass < n_sub_passes; sub_pass++ )
			{
				if( sub_pass == 1 && pass == 0 )
				{
					a_start = 0;
					a_stop = nuse;
				}
				for( int ax = a_start; ax < a_stop; ax++ )
				{
					double x = cur_set[3*ax+0];
					double y = cur_set[3*ax+1];
					double z = (cur_set[3*ax+2]-com[2]) * Lc;
		
					if( pass == 1 && x_atom_list[ax] ) continue;	
					...
					if( sub_pass == 0 )
					{
						if( pass == 0 )
						{
						// using tail beads, hopefully leaflet is irrelevant.
					...
					//		printf("ok.\n");
						...
						for( int iqy = 0; iqy < N_QY; iqy++ )
						{
							...
							double qtot = sqrt(qx*qx+qy*qy);

							if( fabs(qtot) < 1e-5 ) continue;
							if( qtot > q_lim ) continue;
	
							h_FT += 0.5*cos( qx * x * La + qy * y * Lb) * hq[(iqx*N_QY+iqy)*2+0] ; 
							h_FT -= 0.5*sin( qx * x * La + qy * y * Lb) * hq[(iqx*N_QY+iqy)*2+1] ; 
						}
	
				//		if( f == 77 ) printf("C %lf %lf %lf\n", x*La, y*Lb, h_FT );
						...
						if( dz < 0 && !strcasecmp( at[atom_list[ax]].resname, "CHOL") )
						{
							if( leaflet[ax] == 1 )
							{
							//	printf("Flip! %s_%d from + to - z: %lf h_FT: %lf, wrapped dz: %le\n", at[atom_list[ax]].resname, at[atom_list[ax]].res, z, h_FT, dz );
							}
							leaflet[ax] = -1;
						}
						else if( dz > 0 && !strcasecmp( at[atom_list[ax]].resname, "CHOL") )
						{	
							if( leaflet[ax] == -1 )
							{
							//	printf("Flip! %s_%d from - to + z: %lf h_FT: %lf, wrapped dz: %le\n", at[atom_list[ax]].resname, at[atom_list[ax]].res, z, h_FT, dz );
				...
				if( sub_pass == 0 )
				{	// compute Fourier transform only on first sub-pass.
					for( int iqx = 0; iqx < N_QX; iqx++ )
					for( int iqy = 0; iqy < N_QY; iqy++ )
					{
						double qx = 2 * M_PI * (-dqx+iqx) / La;
