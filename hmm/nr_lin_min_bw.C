...
/*			if( w[ttp] > 50.0 ) */
		...
		for( int GE = 0; GE < the_local_system->emission_groups_len[EG]; GE++ )
			the_local_system->p_obs[o*nstates+the_local_system->emission_groups[EG][GE]] = (1-fp)*p[o] + fp * the_local_system->p_obs_BaumWelch[o*nstates+the_local_system->emission_groups[EG][GE]];
	...
	for( int TG = 0; TG < the_local_system->nTransitionGroups; TG++ )
	{
		...
		for( int GE = 0; GE < the_local_system->transition_groups_len[TG]; GE++ )
		{
			...
			for( int o = 0; o < nt; o++ )
			{
				the_local_system->t_ab[s_from*nstates+trans_list[o]] = (1-fp)*p[o] + fp * 
...
//				printf("Parameter EG: %d obs: %d in error, p_temp: %le fp: %le\n", p_temp, fp );
	...
	for( int o = 0; o < nstates; o++ )
	{
		...
		for( int sx = 0; sx < ntrans; sx++, tp++ )
		{
			...
			if( !(w[tp] < 1) && !(w[tp]>0) )
			{
				printf("Parameter EG: %d obs: %d in error, p_temp: %le\n", p_temp );
				if( do_crash_on_bad )
					exit(1);
				return 0;
			}
